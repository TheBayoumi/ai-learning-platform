...
def _existing(identifier: str, *, weight: int) -> CompetencyDefinition:
    source = next(item for item in PYTHON_BACKEND_ROLE.competencies if item.identifier == identifier)
...
LLM_APPLICATIONS: Final = CompetencyDefinition(
    "llm-applications",
    "LLM application engineering",
    "ai",
    "Build bounded model-backed features with explicit prompts, schemas, failure modes, and cost limits.",
    16,
    (
        _activity(
            "Build a structured LLM feature",
            "Wrap a model call behind a typed contract with bounded context and deterministic validation.",
            "A small model-backed service, typed response schema, tests, and failure policy.",
        ...
        _activity(
            ...
            "Make an AI feature degrade safely when a provider is slow, unavailable, or malformed.",
...
RAG: Final = CompetencyDefinition(
    "rag",
    ...
    (
        _activity(
            ...
            "Retrieve a small corpus, rank evidence, and generate an answer with source attribution.",
        ...
        _activity(
            ...
            "A labeled query set, retrieval metrics, error taxonomy, and one validated improvement.",
...
AI_EVALUATION: Final = CompetencyDefinition(
    "ai-evaluation",
    "AI evaluation and safety",
    "ai",
    "Evaluate model-backed behavior with task-specific datasets, rubrics, adversarial cases, and regressions.",
    17,
    (
        _activity(
            "Create an AI regression suite",
            "Define task-level pass criteria and test a model-backed feature against representative cases.",
            ...
            "Results are reproducible enough to compare revisions",
            minutes=150,
        ),
        _activity(
            "Audit an AI feature for unsafe authority",
            "Trace every place model output can affect state and add deterministic validation boundaries.",
...
AI_APPLICATION_ROLE: Final = RoleDefinition(
    ...
    competencies=(
        _existing("python", weight=12),
        _existing("fastapi", weight=8),
        _existing("testing", weight=9),
        _existing("git", weight=5),
        _existing("docker", weight=5),
        _existing("debugging", weight=7),
        _existing("communication", weight=5),
        LLM_APPLICATIONS,
        RAG,
        AI_EVALUATION,
    ),
...
DATA_MODELING: Final = CompetencyDefinition(
    "data-modeling",
    "Analytical data modeling",
    "data",
    "Turn business concepts into durable analytical models with explicit grain, keys, and lineage.",
    12,
    (
        _activity(
            "Design an analytics model",
            "Translate a business workflow into facts, dimensions, keys, and documented grain.",
            ...
            "The model supports the stated queries without hidden assumptions",
            minutes=120,
        ),
        _activity(
...
DATA_PIPELINES: Final = CompetencyDefinition(
    "data-pipelines",
    "Data pipelines and orchestration",
    "data",
    "Build idempotent batch or streaming workflows with observable retries and bounded backfills.",
    13,
    (
        _activity(
            "Build an idempotent ingestion pipeline",
            "Ingest raw events into modeled tables without duplicating data on retry.",
            "Pipeline code, checkpoint strategy, failure injection tests, and runbook.",
...
DATA_QUALITY: Final = CompetencyDefinition(
    "data-quality",
    ...
    (
        ...
        _activity(
            ...
            "Use lineage and validation evidence to isolate where a published metric became incorrect.",
...
DATA_ENGINEER_ROLE: Final = RoleDefinition(
    ...
    competencies=(
        _existing("python", weight=12),
        _existing("postgresql", weight=15),
        _existing("testing", weight=8),
        _existing("git", weight=5),
        _existing("docker", weight=6),
        _existing("ci", weight=6),
        _existing("debugging", weight=8),
        _existing("communication", weight=5),
        DATA_MODELING,
        DATA_PIPELINES,
        DATA_QUALITY,
