...
def _state() -> dict[str, object]:
    return {
        ...
        "phases": [
            _phase(identifier, "PASSED" if identifier == "G01" else "VALIDATING" if identifier == "G02" else "NOT_STARTED")
...
def test_cli_returns_stable_failure_payload(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cli_reports_active_phase(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    phases = (
